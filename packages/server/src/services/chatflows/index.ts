...
const saveChatflow = async (
    ...
    try {
        ...
        // Inject default Langfuse analytics if not set
        if (!newChatFlow.analytic) {
            newChatFlow.analytic = JSON.stringify(DEFAULT_LANGFUSE_ANALYTIC(''))
        }
        ...
        if (containsBase64File(newChatFlow)) {
            ...
            let step1Results = await appServer.AppDataSource.getRepository(ChatFlow).save(chatflow)

            // update analytic with real chatflow id
            step1Results.analytic = JSON.stringify(DEFAULT_LANGFUSE_ANALYTIC(step1Results.id))
            step1Results = await appServer.AppDataSource.getRepository(ChatFlow).save(step1Results)
            ...
            // update analytic with real chatflow id
            dbResponse.analytic = JSON.stringify(DEFAULT_LANGFUSE_ANALYTIC(dbResponse.id))
            dbResponse = await appServer.AppDataSource.getRepository(ChatFlow).save(dbResponse)
        }
...
const updateChatflow = async (
    ...
    try {
        ...
        if (!updateChatFlow.analytic) {
            updateChatFlow.analytic = JSON.stringify(DEFAULT_LANGFUSE_ANALYTIC(chatflow.id))
        }
...
const DEFAULT_LANGFUSE_ANALYTIC = (chatflowId: string) => ({
    langFuse: {
        status: true,
